...
export interface P2PNodeOptions {
  privateKeyHex?: string; // Optional custom private key. Generates a new one if missing.
  listenAddrs?: string[];
...
export class P2PNode {
  public node: any;
  public registry = new ActiveStateRegistry();
  public did: string;
   ...
   */
  async start(options: P2PNodeOptions = {}): Promise<void> {
    ...
    this.node.handle('/daup/handshake/1.0.0', async (incoming: any, connection?: any) => {
      let stream = incoming;
      let conn = connection;
    ...
    this.node.addEventListener('connection:open', async (evt: any) => {
  ...
  subscribeToTopic(
    topic: string,
    onMessage: (payload: string, envelope: MessageEnvelope) => void
  ): void {
    if (!this.node) {
      throw new Error('Node is not started');
    }
   ...
   */
  async registerPeerToDht(peerIdObj: any): Promise<void> {
    try {
      ...
      while (Date.now() - startTime < 8000) {
        ...
        if (peer && peer.protocols.includes(dhtProtocol)) {
          const peerData = {
            id: peerIdObj,
            multiaddrs: peer.addresses.map((a: any) => a.multiaddr),
