 ...
 */
export function getAppManifestSigningPayload(manifest) {
    return [
        manifest.appId,
        manifest.version,
        manifest.name,
 ...
 */
export function signAppManifest(manifest, privateKeyHex) {
 ...
 */
export function verifyAppManifestSignature(manifest) {
...
export class DaupP2PRegistryNode extends P2PNode {
     ...
     */
    async publishAppManifest(manifest) {
        if (!this.node) {
     ...
     */
    async fetchAppManifest(cidOrKey) {
        if (!this.node) {
            throw new Error('P2P node is not started');
        }
        const dht = this.node.services.dht;
     ...
     */
    async broadcastAppRelease(manifest) {
     ...
     */
    async broadcastSubscriptionRenewal(renewal) {
     ...
     */
    async registerDidDocument(did, publicKeyHex) {
        if (!this.node) {
            throw new Error('P2P node is not started');
        }
     ...
     */
    async verifyDidChallenge(did, challenge, signature) {
        try {
